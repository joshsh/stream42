...
import edu.rpi.twc.rdfstream4j.core.Bindings;
import edu.rpi.twc.rdfstream4j.core.LList;
import edu.rpi.twc.rdfstream4j.core.Query;
import edu.rpi.twc.rdfstream4j.core.QueryIndex;
import edu.rpi.twc.rdfstream4j.core.Term;
...
/**
 * Concrete implementation of a RDFStream4j continuous SPARQL query engine.
 ...
 * Optionally (depending on {@link edu.rpi.twc.rdfstream4j.RDFStream4j} settings),
...
public class QueryEngineImpl implements QueryEngine {
    ...
    public long get(final Quantity quantity) {
        if (!RDFStream4j.getDoPerformanceMetrics()) {
    ...
    private void handleCandidateSolution(final String id,
        ...
        // note: RDFStream4j's response to an ASK query which evaluates to true is an empty BindingSet
    ...
    private void logEntry() {
        if (RDFStream4j.getDoPerformanceMetrics()) {
            if (!RDFStream4j.getDoUseCompactLogFormat() || logHasChanged) {
