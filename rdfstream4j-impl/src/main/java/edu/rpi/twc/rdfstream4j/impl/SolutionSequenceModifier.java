    ...
    public boolean trySolution(final BindingSet solution,
        ...
        if (null != distinctSet) {
            if (distinctSet.contains(solution)) {
                ...
                // this is REDUCED, which in RDFStream4j only allows the distinct set to grow to a specified size
                // Currently, the set is simply cleared when it overflows
                if (hasReduced) {
                    if (distinctSet.size() >= RDFStream4j.getReducedModifierCapacity()) {
                        distinctSet.clear();
                    }
                }
